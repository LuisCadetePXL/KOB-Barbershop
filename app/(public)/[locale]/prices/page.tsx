  ...
  return (
    <>
      ...
      <section className="mx-auto max-w-4xl px-4 py-20 sm:px-6">
        <p className="text-center text-kob-muted mb-14">{t('intro')}</p>

        {services && services.length > 0 ? (
          <div className="flex flex-col divide-y divide-kob-border border border-kob-border">
            {services.map((service) => (
              <ServiceRow key={service.id} service={service} locale={locale} minutesLabel={t('minutes')} />
            ))}
          </div>
        ) : (
        ...
        <p className="mt-10 text-center text-xs text-kob-muted">{t('disclaimer')}</p>
      </section>
    </>
  )
}
...
function ServiceRow({
  ...
  // Pick the locale-specific description if filled in; fall back to EN
  const description =
  ...
  return (
    <div className="flex flex-col gap-2 bg-kob-dark px-6 py-5 transition-colors hover:bg-kob-surface sm:flex-row sm:items-center sm:justify-between sm:gap-8">
